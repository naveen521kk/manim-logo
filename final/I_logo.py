...
with cairo.SVGSurface("i_logo.svg",  WIDTH*PIXEL_SCALE,HEIGHT*PIXEL_SCALE) as surface:
    ...
    ctx.move_to(2.5/2, 2.5+(2.5)/2)
    ctx.line_to(2.5+(2.5)/2, 2.5/2)
    ctx.set_source_rgb(0.8, 0.5, 0.7) #color
    ctx.stroke()
    surface.write_to_png('i_logo.png')
import os
os.startfile("i_logo.svg")
